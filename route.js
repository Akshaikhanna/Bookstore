...
con.connect((err) => {
  ...
  // ----------delete----------
  app.delete("/delete/:authorId", (req, res) => {
    const authorIdToDelete = req.params.authorId;
  ...
  app.put("/update/:authorId", (req, res) => {
    ...
    if (!newBirthYear) {
      ...
      return res.status(400).send({ message: "New birth year is required" });
    }
  ...
  app.delete("/deleteBook/:bookTitle", (req, res) => {
    ...
    con.beginTransaction((err) => {
      ...
      const deleteBookQuery = `
        ...
        WHERE title = ?;
      `;
      con.query(deleteBookQuery, [bookTitle], (err, data) => {
        if (err) {
          con.rollback(() => {
